new class e{constructor({position:e="bottom-left"}={}){this.position=this.getPosition(e),this.open=!1,this.initalise()}getPosition(e){let[i,d]=e.split("-");return{[i]:"30px",[d]:"30px"}}async initalise(){async function e(){let e=await fetch("https://api.ipify.org/?format=json").then(e=>e.json());return e}var i=await e();async function d(e){var i="https://app.rfrd.io/read/ip/"+e;console.log("serverURL:",i);let d=await fetch(i).then(e=>e.json());return d}async function t(e){let i=await fetch("https://app.rfrd.io/read/widget/"+e).then(e=>e.json());return i}console.log("foundip:",i.ip),await d(i.ip);let s=document.getElementById("rfrd-widget"),a=s.dataset.id;var n=await t(a);let r=document.createElement("div");r.classList.add("rfrd-cheer-wrapper"),r.style="height: 0px",this.container=r,document.body.appendChild(r);let l=document.createElement("div");l.classList.add("rfrd-cheer-video-bg"),this.buttonContainer=l;let o=document.createElement("div");o.classList.add("rfrd-cheer-background-video");let c=document.createElement("div");c.classList.add("rfrd-cheer-bg","rfrd-background-video","w-background-video-atom"),c.setAttribute("data-video-urls",n.videos[0].videoid.link),c.setAttribute("data-autoplay","true"),c.setAttribute("data-loop","true"),c.setAttribute("data-wf-ignore","true"),this.videogif=c;let p=document.createElement("video");p.style="background-image:url('"+n.videos[0].videoid.screengrab+"')",p.autoplay=!0,p.loop=!0,p.muted=!0,p.playsinline=!0,this.video=p,this.video.innerHTML='<source src="'+n.videos[0].videoid.link+'" data-wf-ignore="true">',this.videogif.appendChild(this.video),o.appendChild(this.videogif);let v=document.createElement("div");v.classList.add("rfrd-cheer-cta"),v.style.backgroundColor="#"+n.maincolor,this.cheercta=v,this.cheercta.innerHTML="<div>"+n.widgetcta+"</div>",this.messageContainer=document.createElement("div"),this.messageContainer.classList.add("rfrd-hidden","rfrd-activated"),this.createMessageContainerContent(n),l.appendChild(o),l.appendChild(this.cheercta),l.addEventListener("click",this.toggleOpen.bind(this)),this.container.append(this.messageContainer),this.container.append(l),this.createStyles(n)}createStyles(e){let i=document.createElement("style");i.innerHTML=`
          .rfrd-cheer-wrapper {
            position: fixed;
            left: 0%;
            top: auto;
